...
class VulkanShaderApp 
{

public: 
	bool bRotating = true;
	string TEXTURE_PATH = "textures/viking_room.png"; 
	string fragShaderPath = "frag.spv";
	string vertShaderPath = "vert.spv";
...
private:
	void initWindow()
	{
		...
		window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan magic zone", nullptr, nullptr);
	...
	void createInstance()
	{
		...
		appInfo.pApplicationName = "Vulkan Shader";
		...
		void* pUserData)
	{
		if (messageSeverity >= VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT)
		{
			...
			cerr << "Warning: validation layer: " << pCallbackData->pMessage << endl;
		}
		if (messageType >= VK_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT)
		{
			// Something has happened that violates the specification or indicates 
			// a possible mistake
			cerr << "Violation or mistake: validation layer: " << pCallbackData->pMessage << endl;
		}
		return VK_FALSE;
	}
	...
	bool isDeviceSuitable(VkPhysicalDevice device)
	{
		...
		return deviceProperties.deviceType ==
			...
			deviceFeatures.geometryShader && hasDeviceQueueFamily(device)
	...
	bool hasDeviceQueueFamily(VkPhysicalDevice device)
	{
	...
	{
		auto vertShaderCode = FileToString(vertShaderPath);
		auto fragShaderCode = FileToString(fragShaderPath);
	...
	string FileToString(const filesystem::path& path)
	{
		const auto root = filesystem::current_path();
		const auto filePath = root / path;
		const bool exists = filesystem::exists(filePath);
		if (!exists)
		{
	...
	void createFramebuffers()
	{
		...
		{
			array<VkImageView, 3> attachments = { colourImageView, depthImageView,
	...
	void drawFrame()
	{
		...
		VkResult result = vkAcquireNextImageKHR(vkDevice, swapChain,
	...
	void createSyncObjects() 
	{
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
		{
			if (vkCreateSemaphore(vkDevice, &semaphoreInfo, nullptr,
				...
				vkCreateFence(vkDevice, &fenceInfo, nullptr,
	...
	{
		vkDestroyImageView(vkDevice, colourImageView, nullptr);
		vkDestroyImage(vkDevice, colourImage, nullptr);
		vkFreeMemory(vkDevice, colourImageMemory, nullptr);
	...
	{
		auto app = reinterpret_cast<VulkanShaderApp*>(glfwGetWindowUserPointer(window));
	...
	void createIndexBuffer() 
	{
		...
		void* data;
		vkMapMemory(vkDevice, stagingBufferMemory, 0, bufferSize, 0,
			&data);
	...
	void updateUniformBuffer(uint32_t currentImage) 
	{
		...
		if (bRotating)
		{
			ubo.model = rotate(mat4(1.0f), time * radians(30.0f), vec3(0.0f, 0.0f, 1.0f));
		}
		else
		{
			ubo.model = rotate(mat4(1.0f), time * radians(0.0f), vec3(0.0f, 0.0f, 1.0f));
		}
	...
	void createDescriptorSets() 
	{
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) 
		{
			...
			descriptorWrites[1].dstBinding = 1; // should this be 1, org was
	...
	void createTextureImage()
	{
		...
		mipLevels = static_cast<uint32_t>(floor(std::log2(max(texWidth, texHeight)))) + 1;
	...
	VkFormat findSupportedFormat(const vector<VkFormat>& candidates, VkImageTiling 
	...
	void createColourResources()
	{
		...
		createImage(swapChainExtent.width, swapChainExtent.height, 1, msaaSamples, 
			colourFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT
